...
export class ConfirmPage {
  ...
  constructor(
    ...
    this.wallet = this.spvNodeProvider.getWallet();
  }
  ...
  ionViewWillEnter() {
    ...
    this.tx.feeLevelName = this.feeProvider.getFeeOpts[this.tx.feeLevel];
    ...
    this.setButtonText(false, false);
  }
      ...
      this.feeProvider
        ...
        .then((feeRate: any) => {
          ...
          if (this.usingCustomFee) {
            ...
            tx.feeLevelName = this.feeProvider.getFeeOpts[tx.feeLevel];
            tx.feeRate = feeRate;
          }
  ...
  public approve(tx: any, wallet: any): Promise<void> {
    ...
    wallet
      .createTX(
        // 创建交易
        txOptions // 包含地址和金额的交易内容
      )
      .then(mtx => {
        // 创建交易成功
        tx = mtx; // 记录交易对象句柄
        return wallet.sign(tx); // 开始签名，返回了一个Promise
      })
      .then(() => {
        // 签名成功
        this.logger.info('ready to convert: ', tx); // 显示 mutable tx
        tx = tx.toTX(); // 将 mutable tx 转化为 immutable tx
        this.logger.info('ready to send: ', tx); // 显示 immutable tx
        return this.spvNodeProvider.sendTX(tx); // 发送交易到网络，返回了一个Promise
      })
      .then(() => {
        this.onGoingProcessProvider.clear(); // 发送成功
